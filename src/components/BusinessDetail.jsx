...
// Asegúrate de que esta ruta sea correcta para apiService
// Y que apiService tenga una función getBusinessById
...
import {
    ...
    FaClock, FaInfoCircle, FaArrowLeft, FaTag, FaStore, FaUserCheck, // Nuevo icono para miembro premium
    FaTiktok
} from 'react-icons/fa';

// Se utiliza la variable de entorno para la URL base de la API.
// Proporciona un valor predeterminado si la variable no está definida.
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000';

function BusinessDetail() {
    const { id } = useParams(); // Obtiene el ID del negocio de la URL
    const navigate = useNavigate(); // Inicializa useNavigate para volver atrás
    ...
    useEffect(() => {
        const fetchBusinessDetails = async () => {
            ...
            try {
                ...
                console.error("Error al cargar los detalles del negocio:", err);
                if (err.response && err.response.status === 404) {
                    setError("Negocio no encontrado.");
                } else if (err.response && (err.response.status === 401 || err.response.status === 403)) {
                    setError("No tienes permiso para ver este negocio o tu sesión ha expirado.");
                } else {
        ...
        if (id) {
            ...
            setError("No se proporcionó un ID de negocio.");
            setLoading(false);
        }
    }, [id]);
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-indigo-100 to-purple-100 flex-col">
                ...
                <p className="text-xl text-indigo-800 font-semibold drop-shadow-sm">Cargando detalles del negocio...</p>
            </div>
        );
    }
    ...
    if (!business) {
        return (
            <div className="flex justify-center items-center min-h-screen bg-gray-50 p-6 flex-col text-center">
                ...
                <h2 className="text-4xl font-extrabold text-gray-800 mb-4">Negocio No Disponible</h2>
                <p className="text-xl text-gray-600 max-w-md">El negocio que buscas no existe o ha sido eliminado.</p>
                <button
                    onClick={() => navigate('/')} // Puedes redirigir a la página principal o a una lista de negocios si tuvieras
    ...
    const {
        name, logo, street_address, municipality,
        contact_phone, hours, what_they_sell, business_type, is_paid_member,
        social_media_facebook_username, social_media_instagram_username, social_media_tiktok_username
    } = business;

    console.log(social_media_tiktok_username)
    // Se utiliza la variable de entorno para construir la URL del logo
    const businessLogoUrl = logo && (logo.startsWith('http') ? logo : `${API_BASE_URL}${logo}`);
    const uriangatoLocation = "Uriangato, Guanajuato, México"; // Asumiendo que todos los negocios están aquí
    ...
    return (
        <div className="relative container mx-auto p-4 md:p-8 bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen flex items-center justify-center">
            ...
            <div className="max-w-5xl mx-auto bg-white rounded-3xl shadow-3xl overflow-hidden transform transition-all duration-300 ease-in-out hover:shadow-4xl border border-gray-100 animate-slide-up p-6 sm:p-10 md:p-12">
                {/* Sección del Encabezado del Negocio */}
                <section className="mb-10 pb-8 border-b-2 border-indigo-100 text-center">
                    {businessLogoUrl ? (
                        <div className="mb-6">
                            <img
                                src={businessLogoUrl}
                                alt={`Logo de ${name}`}
                                className="w-48 h-48 object-contain rounded-full shadow-xl border-4 border-indigo-200 mx-auto transform hover:scale-105 transition-transform duration-300 ease-in-out"
                            />
                        </div>
                    ) : (
                        <div className="w-48 h-48 bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center text-indigo-600 text-7xl font-extrabold rounded-full mx-auto mb-6 shadow-xl border-4 border-indigo-200">
                            {name ? name.charAt(0).toUpperCase() : 'N/A'}
                        </div>
                    )}

                    <h1 className="text-5xl font-extrabold text-indigo-900 leading-tight mb-4">
                        {name}
                    </h1>

                    {is_paid_member && (
                        <span className="inline-flex items-center bg-green-100 text-green-800 text-sm font-semibold px-4 py-2 rounded-full shadow-md mb-6">
                            <FaUserCheck className="mr-2 text-green-600" /> Miembro Premium
                        </span>
                    )}
                ...
                <section className="mt-10 pt-8 border-t-2 border-gray-100">
                    <h2 className="text-4xl font-bold text-indigo-900 text-center mb-8">
                        <FaInfoCircle className="inline-block mr-4 text-indigo-600 text-4xl" />
                        Información Clave
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-800 text-lg mb-8">
                        {what_they_sell && (
                            <p className="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <strong className="font-semibold mr-2">Ofrecen:</strong>
                        ...
                        {business_type && (
                            <p className="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm">
                                <FaStore className="mr-4 text-orange-500 text-2xl flex-shrink-0 mt-1" />
                                <strong className="font-semibold mr-2">Giro:</strong>
                        ...
                        <p className="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm">
                            ...
                            <span>{street_address}, {municipality}, {uriangatoLocation}</span>
                        </p>
                        {contact_phone && (
                            <p className="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm">
                    ...
                    <section className="mt-8 pt-6 border-t border-gray-200">
                        ...
                        <div className="flex gap-6 justify-center">
                            ...
                            {social_media_tiktok_username && (
                                <a href={`https://tiktok.com/${social_media_tiktok_username}`} target="_blank" rel="noopener noreferrer" className="text-gray-800 hover:text-gray-900 text-5xl transform hover:scale-110 transition-transform duration-300 ease-in-out" aria-label="Twitter">
                                    <FaTiktok />
                                </a>
                            )}
                        </div>
